...
export async function exportToExcel(
  ...
  const res = await fetch(`${BASE}/export`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ job_id: jobId, format: "excel", results, remarks, purpose }),
  });
...
export async function exportToSheets(
  ...
  const res = await fetch(`${BASE}/export`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ job_id: jobId, format: "sheets", results, sheet_id: sheetId, remarks, purpose }),
  });
  if (!res.ok) {
    const err = await res.json();
