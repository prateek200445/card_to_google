...
export default function UploadZone({ onFiles, isUploading, uploadPct, disabled, onViewAll }: Props) {
  ...
  return (
    <div className="w-full">
      {previews.length === 0 ? (
        /* Bento Grid Layout matching code.html */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl mx-auto">
          
          {/* Card 1: Upload */}
          <div
            {...getRootProps()}
            className={`bg-white border border-surface-border rounded-2xl p-10 flex flex-col items-center justify-center min-h-[340px] bouncy-hover cursor-pointer group relative overflow-hidden shadow-lg transition-all duration-300
              ${isDragActive ? "border-primary bg-primary/5 scale-[1.01]" : "hover:border-primary/30"}
              ${(disabled || isUploading) ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            <input {...getInputProps()} />
            <div className="w-20 h-20 rounded-full bg-surface-container flex items-center justify-center mb-6 group-hover:bg-primary/10 transition-colors duration-300">
              <Upload className="w-8 h-8 text-primary stroke-[1.5]" />
            </div>
            <h3 className="font-headline text-2xl font-bold text-on-surface mb-3">Upload files</h3>
            <p className="text-center text-on-surface-variant text-sm mb-8 leading-relaxed font-body">
              Drag & drop or <span className="text-primary hover:underline underline-offset-4 cursor-pointer font-semibold">browse</span>
              <br />
              <span className="text-xs mt-2 block uppercase tracking-wider text-on-surface-variant/70">JPG, PNG</span>
            </p>
            <button
              type="button"
              className="bg-white border border-surface-border text-on-surface px-8 py-2.5 rounded-full font-semibold text-sm hover:border-primary hover:text-primary transition-all active:scale-95 shadow-sm"
            >
              Select Files
            </button>
          </div>
          ...
          <div
            ...
            className={`bg-primary text-white rounded-2xl p-10 flex flex-col items-center justify-center min-h-[340px] bouncy-hover cursor-pointer relative overflow-hidden group shadow-lg shadow-primary/20
              ${(disabled || isUploading) ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            <div className="w-20 h-20 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500">
              <Camera className="w-8 h-8 text-white stroke-[1.5]" />
            </div>
            <h3 className="font-headline text-2xl font-bold mb-3 text-white">Camera</h3>
            <p className="text-center text-white/80 text-sm mb-8 leading-relaxed max-w-[200px] font-body">
              Take a photo of a business card instantly.
            </p>
            <button
              type="button"
              className="bg-white text-primary px-8 py-3 rounded-full font-bold text-sm w-full hover:bg-white/95 transition-all active:scale-95 flex items-center justify-center gap-1.5 shadow-sm"
            >
              Open Camera
          ...
          <div className="bg-white border border-surface-border rounded-2xl p-8 flex flex-col min-h-[340px] bouncy-hover shadow-lg">
            <div className="flex justify-between items-center mb-6 border-b border-surface-border pb-4">
              <h3 className="font-headline text-xl font-bold text-on-surface flex items-center gap-2">
                <BookUser className="w-5 h-5 text-primary stroke-[1.5]" />
                Recent
              </h3>
              <button
                type="button"
              ...
              {recent.map((c, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={onViewAll}
                    className="flex items-center gap-3.5 p-2.5 rounded-xl hover:bg-surface-container transition-all duration-200 cursor-pointer group"
                  >
                    <div className="w-11 h-11 rounded-full bg-surface-container border border-surface-border flex items-center justify-center text-primary font-headline font-bold text-sm group-hover:border-primary/45 transition-colors shrink-0">
                      {getInitials(c.name)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="font-headline font-bold text-on-surface text-sm truncate">
                        {c.name || "Unknown"}
                      </h4>
        ...
        /* Preview Grid State (styled to match new light-themed style) */
        <div className="max-w-4xl mx-auto bg-white border border-surface-border rounded-3xl p-8 shadow-xl space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold text-on-surface">Selected Cards</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            ...
            {/* Click to add more card inside grid */}
            {previews.length < MAX && (
              <div
                {...getRootProps()}
          ...
          {isUploading ? (
            ...
            <button
              ...
              className="w-full py-4 rounded-full font-bold text-white bg-primary hover:bg-primary/95 transition-all active:scale-[0.98] shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
            >
